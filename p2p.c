...
time_t convertToTime(const char *timestamp);
void sortPeers();
int compareResponses(const void *a, const void *b);
void broadcastUpdate(const char *myTimeStr, const char *myMAC, const char *myIP,
...
void UpdatePeers(const char *timeStr, const char *mac, const char *ip,
    ...
    while(fgets(line,sizeof(line),fp)){
        ...
        char *availStr=strtok(NULL, "|");
         ...
         if(fileList && strlen(fileList) >0){
            ...
            UpdatePeers(timeStr,mac,ip,name,total,avail,filecount,filenames);
         addedCount++;
    }
...
{ 
    ...
    FILE *fp = popen(
        "ip link show 2>/dev/null | grep -A1 'state UP' | grep ether | awk '{print $2}' | head -1 || "
        "cat /sys/class/net/$(ip route show default | awk '/default/ {print $5}' | head -1)/address 2>/dev/null || "
        "ifconfig 2>/dev/null | grep -m 1 ether | awk '{print $2}'",
        "r");
...
void UpdatePeers(const char *timeStr,const char *mac,const char *ip,const char *name,
long long total,long long avail,int fileCount,char files[][128]){
    if(!mac || !ip) return;    
...
int comparePeerByTime(const void *a,const void *b){
    const Peer *pa = (const Peer*) a;
    const Peer *pb = (const Peer*) b;
    time_t ta=convertToTime(pa->timeStr);
    time_t tb = convertToTime(pb->timeStr);
    if(ta>tb) return -1;
    if(ta<tb) return 1;
    return 0;
}
void sortPeers(){
    qsort(peerList,peerCount,sizeof(Peer),comparePeerByTime);
...
void getPeersSortByAvail(int sorted[],int count){
...
void findBestPeer(char **bestIP,int *peerIndex){
    ...
    getMyMAC(myMAC);
    int order[MAX_PEERS];
    getPeersSortByAvail(order,peerCount);
    int flag=0;
    int i;
...
void handleIncomingMessage(const char*fromIP,char *message)
{
    ...
    if (strcmp(command,"I_AM_NEW")==0)
    {
        ...
        if (mac && ip && name && totalStr && availStr)
        {
            ...
            UpdatePeers(timeStr,mac,ip,name,total,avail,0,emptyFiles);
        }
    ...
    if (strcmp(command,"HAVE_UPDATED_ONE")==0)
    {
        ...
        upresp[uprespCount].timeT=convertToTime(time);
    ...
    if (strcmp(command,"UPDATE_LINE")==0)
    {
        ...
        for (int i=0; i<filecount&&i <MAX_FILES;i++)
        {
               ...
               if (f)
            {
             ...
             UpdatePeers(time, mac, ip, name, total, avail, filecount, filenames);
        return;
    }
   ...
   if (!time||!peercountStr)
        ...
        time_t myLastTime = convertToTime(tempMyLastTime);
        time_t timeAsk = convertToTime(time);
...
void sync()
{
 ...
 sendUnicastMessage(winnerIP, "I_NEED_PEERSTXT", DISCOVERY_PORT);
    ...
    {
        printf(" Failed to receive peers.txt\n");
    }
}
...
void writeMyInfo(const char *s1,
    const char *s2,
    const char *s3,
    const char *s4,
    long long n1,
  ...
  int main(){
    ...
    if(!alreadyPeer){
        ...
        UpdatePeers(currentTime, myMAC, myIP, myName, myTotal, myAvail, filecount, files);
        char final[256];
